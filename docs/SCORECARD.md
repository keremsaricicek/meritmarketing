...
|---|---|---|---|
| 1 | Product preservation | 9/10 | No screen redesigned. The 529 baseline assertions still run against `merit-marketing-hub.html`. Where a reviewer proposed changing behaviour — scoping the guest inspector's counts — the prototype's own `custRes` helper was checked first, found unscoped, and the *list* was widened to match the counts instead. Losing a point because the notification producers were missing until the review found them: that was a real behavioural regression from the baseline, live in the tree for weeks. |
...
| 12 | Test quality and regression protection | 8/10 | 33 suites, 1131 assertions, on the production runtime. The security matrix is load-bearing: a verb listed as record-scoped with no probe registered is a failure. But this is the dimension the review hurt most — 1016 assertions could not see that the application did not start, and four assertions could not fail at all. Both are fixed; the score reflects that they existed. |
